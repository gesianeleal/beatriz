...
<h1>
	<i class="icon-th-list"></i> Questões
	...
	<script type="text/template" id="questaoCollectionTemplate">
		...
		<thead>
			<tr>
				...
				<th id="header_GrupoquestaoDescricao">Grupo Questão<% if (page.orderBy == 'GrupoquestaoDescricao') { %> <i class='icon-arrow-<%= page.orderDesc ? 'up' : 'down' %>' /><% } %></th>
			</tr>
		</thead>
		<tbody>
		<% items.each(function(item) { %>
			<tr id="<%= _.escape(item.get('id')) %>">
				...
				<td><%= _.escape(item.get('grupoquestaoDescricao') || '') %></td>
			</tr>
		<% }); %>
		</tbody>
		</table>
		...
		<form class="form-horizontal" onsubmit="return false;">
			<fieldset>
				...
				<div id="grupoquestaoidInputContainer" class="control-group">
					<label class="control-label" for="grupoquestaoid">Grupo Questão</label>
					<div class="controls inline-inputs">
		...
		<form id="deleteQuestaoButtonContainer" class="form-horizontal" onsubmit="return false;">
			<fieldset>
				<div class="control-group">
					...
					<div class="controls">
						<button id="deleteQuestaoButton" class="btn btn-mini btn-danger"><i class="icon-trash icon-white"></i> Excluir Questão</button>
						<span id="confirmDeleteQuestaoContainer" class="hide">
							<button id="cancelDeleteQuestaoButton" class="btn btn-mini">Cancelar</button>
							<button id="confirmDeleteQuestaoButton" class="btn btn-mini btn-danger">Confirmar</button>
						</span>
					</div>
				</div>
			</fieldset>
	...
	<div class="modal hide fade" id="questaoDetailDialog">
		<div class="modal-header">
			...
			<h3>
				<i class="icon-edit"></i> Alterar Questão
		...
		<div class="modal-footer">
			...
			<button id="saveQuestaoButton" class="btn btn-primary">Salvar</button>
		</div>
	</div>
	...
	<p id="newButtonContainer" class="buttonContainer">
		<button id="newQuestaoButton" class="btn btn-primary">Inserir Questão</button>
	</p>
